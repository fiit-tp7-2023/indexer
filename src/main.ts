...
processor.run(new TypeormDatabase({supportHotBlocks: true}), async (ctx) => {
    let TransfersERC721: TransferEventERC721[] = []
    ...
    await processTransfersERC721(ctx, TransfersERC721, cache)
...
async function processTransfersERC721(ctx: Context, transfersData: TransferEventERC721[], cache: Cache) {
    let nftsData: Map<string, NftData> = new Map()
    ...
    await getOrCreateNfts(ctx, cache, nftsData);
    transfersData.forEach(transferData => {
...
export async function getOrCreateNfts(ctx: Context, cache: Cache, nftsData: Map<string, NftData>){
    ...
    const collectionsData: Map<string, CollectionData> = new Map()
    ...
    await getOrCreateNftCollections(ctx, cache, collectionsData)
...
export async function getOrCreateNftCollections(ctx: Context, cache: Cache, collectionsData: Map<string, CollectionData>){
